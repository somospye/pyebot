...
import type { Modal } from "seyfert";
...
async function showModal(context: ComponentContext, modal: Modal | null): Promise<void> {
  if (!modal) {
    await context.followup({
      content: "⚠️ No se pudo abrir el modal.",
      flags: 1 << 6,
    });
    return;
  }

  await context.modal(modal.toJSON());
}

export default class RolesDashboardStringSelectHandler extends ComponentCommand {
  ...
  async run(context: ComponentContext<"StringSelect">): Promise<void> {
    ...
    if (parsed.action === "open_option" && selection === "rename") {
      await showModal(context, buildRenameModal(session));
      return;
    }

    const requiresModal = parsed.action === "limits_select_action";
    ...
    if (requiresModal) {
      ...
      if (selection) {
        await showModal(context, buildLimitModal(session, selection));
      } else {
        await context.followup({
          content: "⚠️ Selecciona una accion valida para editar sus limites.",
          flags: 1 << 6,
        });
      }
...
function getValues(context: ComponentContext<"StringSelect">): readonly string[] {
