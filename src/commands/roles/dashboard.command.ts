...
function formatOverrideLabel(override: RoleCommandOverride | undefined): string {
    switch (override) {
        ...
        default: return "Heredar";
    }
}

// Accepts empty -> “no window”, “0*” -> “no window”, only errors on non-empty invalid strings
function normalizeWindowInput(input: string | undefined): LimitWindow | null | "empty" {
...
function windowToSeconds(window: LimitWindow): number {
    ...
    const unit = match[2];
    return unit === "m" ? value * 60 : unit === "h" ? value * 3600 : value * 86400;
}
...
export default class RolesDashboardCommand extends SubCommand {
    async run(ctx: GuildCommandContext) {
        ...
        if (!guildId) {
            await ctx.write({ content: "[!] Este comando solo puede ejecutarse dentro de un servidor." });
            return;
        }
            ...
            (state) => {
                ...
                const limitModal = new Modal()
                    ...
                    .run(async (modalCtx) => {
                        ...
                        const refreshedAction = actions.find((i) => i.key === state.focusedAction) ?? actions[0];
                        ...
                        const windowValue: LimitWindow | null = nw === "empty" ? null : (nw as LimitWindow | null);
                        const windowSeconds = windowValue ? windowToSeconds(windowValue) : null;
                ...
                const clearLimitButton = new Button()
                    ...
                    .onClick("roles_dashboard_clear_limit", async (buttonCtx) => {
                        const targetRoles = findRolesByDiscordIds(state.selectedRoleIds, state.roles);
                        const a = actions.find((i) => i.key === state.focusedAction) ?? actions[0];
