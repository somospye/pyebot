...
const WINDOW_DESCRIPTIONS: Record<LimitWindow, string> = {
    "10m": "cada 10 minutos",
    "1h": "cada hora",
    "6h": "cada 6 horas",
    "24h": "cada 24 horas",
...
function toDashboardRoles(entries: Awaited<ReturnType<typeof listRoles>>): DashboardRole[] {
    return entries.map(({ roleKey, record }) => ({
        key: roleKey,
        label: record.label,
...
function normalizeWindowInput(input: string | undefined): LimitWindow | null | "invalid" {
    if (!input) return null;
    const normalized = input.trim().toLowerCase();
    if (!normalized) return null;
    if ((LIMIT_WINDOWS as readonly string[]).includes(normalized)) {
...
export default class RolesDashboardCommand extends SubCommand {
    async run(ctx: GuildCommandContext) {
        ...
        const fetchRoles = async () => toDashboardRoles(await listRoles(guildId, ctx.db.instance));
            ...
            (state) => {
                ...
                const limitModal = new Modal()
                    ...
                    .run(async (modalCtx) => {
                        ...
                        if (windowResult === "invalid") {
                            await modalCtx.write({
                                content: "Ventana invalida. Usa una de las opciones: 10m, 1h, 6h, 24h o 7d.",
                                flags: MessageFlags.Ephemeral,
                            });
                            return;
                        }
                        ...
                        try {
                            if (limitNumber === 0) {
                                ...
                                const limitRecord: RoleLimitRecord = {
                                    ...
                                    window: windowResult,
                                    windowSeconds: windowResult ? undefined : null,
                                };
                                ...
                                state.feedback = `Limite actualizado para ${refreshedAction.label}: ${limitRecord.limit} uso(s) ${limitRecord.window ? WINDOW_DESCRIPTIONS[limitRecord.window] : "sin ventana"} (${refreshedRoles.length} rol(es)).`;
                            }
                ...
                const clearLimitButton = new Button()
                    ...
                    .onClick("roles_dashboard_clear_limit", async (buttonCtx) => {
                        ...
                        try {
                            for (const role of targetRoles) {
                                await clearRoleLimit(guildId, role.key, action.key, ctx.db.instance);
                            }
